# ND Player — Codex Context

Monorepo holding two **Navidrome** clients that both speak the **Subsonic REST API**.
Full API reference: `docs/subsonic-api.txt` in the repo root.

```
apps/mobile/      Expo / React Native app (Android)
apps/extension/   Chrome extension (Vite + React + React Router)
docs/             Shared reference (Subsonic API)
```

Each app has its own `package.json` — run `npm install` inside the app you are working on.

---

## apps/mobile — tech stack
...
## apps/extension — tech stack
- **Vite** + **React 19** + **react-router-dom**, Manifest V3 (`public/manifest.json`)
- **Zustand** store in `src/store/`, **@tanstack/react-query** for fetching
- Styles in `src/styles/layout.css` and per-component CSS (`src/components/*.css`)
- `npm run dev` for the browser dev server, `npm run build` → `dist/` for the unpacked extension
...
### apps/extension (important files)
