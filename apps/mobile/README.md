...
cd ND-Player/apps/mobile
npm install
```

### Dev credentials (optional)

Create a `.env.local` file in `apps/mobile/` to pre-fill the login screen during development:
...
See [docs/privacy-policy.html](../../docs/privacy-policy.html) — kept at the repo root so the
published URL stays valid:
